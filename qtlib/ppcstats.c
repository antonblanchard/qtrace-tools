...
struct syscall syscalls[] = {
	...
	{ "rseq",               387},
	{ "io_pgetevents",      388},
	{ "UNKNOWN",		389}, /* this must be the last entry */
};
#define NR_SYSCALLS (sizeof(syscalls) / sizeof(struct syscall))
...
/* FIXME: merge with exceptions_sort()) */
static void syscalls_sort(void)
{
	struct syscall sys;
	int i, j;
	bool swap;

	/* stupid bubble sort of syscall table based on count */
	for (i = 0; i < NR_SYSCALLS; i++) {
		swap = false;
		for (j = 0; j < NR_SYSCALLS- 1; j++) {
			if (s.syscalls[j+1].count > s.syscalls[j].count) {
				swap  = true;
				sys = s.syscalls[j+1];
				s.syscalls[j+1] = s.syscalls[j];
				s.syscalls[j] = sys;
			}
		}
		if (!swap)
